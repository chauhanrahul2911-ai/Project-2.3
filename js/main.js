...
// (keyed by the user's UID) so it survives a browser-data-clear or a
// device switch — see js/firebase-config.js for the Firestore setup.
//
...
function firestoreReady() {
    return firebaseReady() && typeof db !== 'undefined' && typeof firebase.firestore === 'function';
}
...
// Pulls this UID's premium status from Firestore (source of truth) and
// re-renders. Runs on every login / page-load-with-existing-session.
// Pulls this UID's premium status from Firestore (source of truth — only
// the Razorpay webhook, functions/index.js, is allowed to write it now)
// and re-renders. Runs on every login / page-load-with-existing-session.
//
...
function syncPremiumFromCloud(uid) {
    if (!firestoreReady()) return;
    db.collection('users').doc(uid).get()
        .then((docSnap) => {
            const cloudPremium = docSnap.exists && docSnap.data().isPremium === true;
        ...
        .catch((error) => {
            ...
            console.error("Could not fetch premium status from Firestore:", error);
        });
}
...
function startRazorpayPayment() {
    ...
    if (!firebaseReady() || !auth.currentUser) {
        ...
        // but the webhook needs a UID to know whose account to unlock.
    ...
    const options = {
        ...
        // this is how the server-side unlock knows WHICH account to credit.
        notes: {
        ...
        handler: function (response) {
            ...
            // "optimistic": Firestore itself can no longer be written from here
            // (see firestore.rules). The Razorpay webhook (functions/index.js),
            // running on Razorpay's own server, is what actually confirms this
            // server-side and writes Firestore moments later — independent of
            // whether this browser stays open or not.
            isPremiumUser = true;
            localStorage.setItem('gsrtc_is_premium', 'true');
