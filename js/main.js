...
function startRazorpayPayment() {
    ...
    const options = {
        ...
        handler: function (response) {
            // Fires ONLY after Razorpay confirms the payment succeeded.
            isPremiumUser = true;
            localStorage.setItem('gsrtc_is_premium', 'true');
            ...
            // Persist to the account (Firestore) so this unlock survives a
            // browser-data-clear or a different device — not just this browser.
            // The local unlock above already happened regardless of this succeeding,
            // so a slow/offline Firestore write never blocks today's access.
            if (firestoreReady() && auth.currentUser) {
                db.collection('users').doc(auth.currentUser.uid).set({
                    ...
                    console.error("Could not sync premium status to Firestore:", error);
                });
            }
        },
        modal: {
